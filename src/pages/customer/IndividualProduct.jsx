...
const fetchRelatedProducts = async (category) =>{
    await axios.get('http://localhost:6001/fetch-products').then(
        (response)=>{
            const related = response.data.filter(p => p.category === category && p._id !== id).slice(0, 4);
...
const handleShare = async() =>{
    ...
    const shareData = {
        title: productName,
        text: `Check out ${productName} - ₹${parseInt(productPrice - (productPrice * productDiscount)/100)}`,
        url: shareUrl
    };
            ...
            <div className="related-products">
                ...
                <div className="related-products-grid">
                    {relatedProducts.map((product)=>(
                        <div className="related-product-card" key={product._id} onClick={()=> navigate(`/product/${product._id}`)}>
                            <img src={product.mainImg} alt={product.title} />
                            <h6>{product.title}</h6>
                            <p>₹{parseInt(product.price - (product.price * product.discount)/100)} <s>₹{product.price}</s></p>
                        </div>
                    ))}
                </div>
            </div>
