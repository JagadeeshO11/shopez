...
const IndividualProduct = () => {
  ...
  const fetchRelatedProducts = async (category) => {
    const response = await axios.get(`${API_BASE}/fetch-products`);
    setRelatedProducts(response.data.filter(p => p.category === category && p._id !== id).slice(0, 4));
  };
  ...
  const buyNow = async () => {
    ...
    try {
      await axios.post(`${API_BASE}/buy-product`, {userId, name, email, mobile, address, pincode, title: productName, description: productDescription, mainImg: productMainImg, size, quantity: productQuantity, price: productPrice, discount: productDiscount, paymentMethod, orderDate: new Date()});
      alert('Order placed!!');
      navigate('/profile');
    } catch {
      alert('Order failed!!');
  ...
  const handleAddToCart = async () => {
    ...
    try {
      await axios.post(`${API_BASE}/add-to-cart`, {userId, title: productName, description: productDescription, mainImg: productMainImg, size, quantity: productQuantity, price: productPrice, discount: productDiscount});
  ...
  const handleShare = async () => {
    ...
    const shareData = {title: productName, text: `Check out ${productName} - ₹${parseInt(productPrice - (productPrice * productDiscount)/100)}`, url: shareUrl};
    if (navigator.share) {
      try {
        await navigator.share(shareData);
        await axios.post(`${API_BASE}/share-product`, {productId: id, productTitle: productName, sharedBy: username, sharedTo: 'Native Share'});
      } catch (err) {
      ...
      if(shareEmail){
        await axios.post(`${API_BASE}/share-product`, {productId: id, productTitle: productName, sharedBy: username, sharedTo: shareEmail})
    ...
    <div className="IndividualProduct-page">
      ...
      <div className="IndividualProduct-body">
        ...
        <div className="IndividualProduct-data">
          ...
          <span><label htmlFor="productSize">Choose size</label><select name="productSize" id="productSize" value={size} onChange={(e)=>setSize(e.target.value)}><option value=""></option>{productSizes.map((itemSize)=><option key={itemSize} value={itemSize}>{itemSize}</option>)}</select></span>
      ...
      {relatedProducts.length > 0 && <div className="related-products"><h3>Related Products</h3><div className="related-products-grid">{relatedProducts.map(product=><div className="related-product-card" key={product._id} onClick={()=>navigate(`/product/${product._id}`)}><img src={product.mainImg} alt={product.title} /><h6>{product.title}</h6><p>₹{parseInt(product.price - (product.price * product.discount)/100)} <s>₹{product.price}</s></p></div>)}</div></div>}
    </div>
  )
}
